/**
 * Parsea y procesa spritesheets Sparrow, aplicando recortes y rotaciones dinamicamente.
 */
class SparrowParser {
	/**
	...
	static fixPhaserSparrow(scene, texKey, xmlText) {
		...
		const sourceImage = texture.getSourceImage(); // Fuente original de la imagen
		const parser = new DOMParser(); // Interprete de texto a nodo
		const xmlDoc = parser.parseFromString(xmlText, "text/xml");
		const subTextures = xmlDoc.getElementsByTagName("SubTexture"); // Elementos individuales de la textura
		
		const tempCanvas = document.createElement('canvas'); // Lienzo de trabajo para transformaciones
		const tempCtx = tempCanvas.getContext('2d');

		for (let i = 0; i < subTextures.length; i++) {
			const node = subTextures[i];
			const name = node.getAttribute("name");
			const rotated = node.getAttribute("rotated") === "true"; // Estado de rotacion del empaquetado
			...
			if (rotated) {
				...
				tempCtx.drawImage(sourceImage, x, y, xmlWidth, xmlHeight, -xmlWidth / 2, -xmlHeight / 2, xmlWidth, xmlHeight);
				
				const frameCanvas = document.createElement('canvas'); // Lienzo dedicado para el frame rotado
				frameCanvas.width = tempCanvas.width;
				frameCanvas.height = tempCanvas.height;
				frameCanvas.getContext('2d').drawImage(tempCanvas, 0, 0);

				if (typeof Phaser !== 'undefined' && Phaser.Textures && Phaser.Textures.TextureSource) {
					const texSource = new Phaser.Textures.TextureSource(texture, frameCanvas); // Origen en memoria para Phaser
					...
					const newFrame = texture.add(name, sourceIndex, 0, 0, tempCanvas.width, tempCanvas.height);
					if (newFrame) {
						newFrame.setTrim(frameWidth, frameHeight, frameX, frameY, tempCanvas.width, tempCanvas.height);
					}
				}
				
			} else {
				let frame = texture.frames[name]; // Búsqueda directa del frame en el objeto
...
if (!funkin.animation) funkin.animation = {};
funkin.animation.sparrow = SparrowParser;