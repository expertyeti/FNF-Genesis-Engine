/**
 * @file src/funkin/play/data/preload/preloadSkins.js
...
class SkinPreloader {
    ...
    /**
     * @param {Phaser.Scene} scene - La escena actual (PlayScene)
     * @param {Object} uiSkinsManager - La instancia de funkin.play.uiSkins
     */
    async preload(scene, uiSkinsManager) {
        return new Promise((resolve) => {
            if (!uiSkinsManager.skinData && !uiSkinsManager.fallbackSkinData) return resolve();

            const mainPath = (uiSkinsManager.skinData && uiSkinsManager.skinData.global && uiSkinsManager.skinData.global.basePath) ? uiSkinsManager.skinData.global.basePath : "Funkin";
            const fallbackPath = (uiSkinsManager.fallbackSkinData && uiSkinsManager.fallbackSkinData.global && uiSkinsManager.fallbackSkinData.global.basePath) ? uiSkinsManager.fallbackSkinData.global.basePath : "Funkin";
            ...
            const loadAsset = (type, originalPath, isAtlas = false, isFallback = false) => {
                ...
                const fileUrl = `public/images/skins/${sourcePath}/${originalPath}`;
            ...
            ['left', 'down', 'up', 'right'].forEach(dir => {
                resolveAsset(`gameplay.holdCovers.directions.${dir}.assetPath`, 'image', true);
            });
...
funkin.play.skinPreloader = new SkinPreloader();