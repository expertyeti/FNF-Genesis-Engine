/**
 * Controls health bar icons and their properties.
 */
class HealthBarIcon {
  /**
   * @param {Phaser.Scene} scene
   * @param {Object} healthBar
  ...
  updateIconTexture() {
    ...
    if (this.currentIcon !== targetIcon) {
      ...
      if (this.scene.textures.exists(iconKey) && this.scene.textures.get(iconKey).key !== "__MISSING") {
        ...
        const iconUrl = `public/images/icons/${targetIcon}.png`;
        ...
        const onError = (file) => {
          if (file.key === iconKey) {
            ...
            // FIX: Ya no sobreescribimos this.currentIcon = "face", sino que 
            // aplicamos la textura de resguardo conservando targetIcon, evadiendo
            // el bucle infinito de intentos de redescarga (spam 404).
            if (this.currentIcon === targetIcon) {
  ...
  updatePosition(healthLerp) {
    ...
    this.sprite.x = anchorCenterX - scaledWidth / 2 + this.iconOffset[0];
  }
...
funkin.play.HealthBarIcon = HealthBarIcon;