...
class ArrowsSpawner {
	...
	static spawnStrumlines(scene, strumlines) {
		...
		if (funkin.playCamera && funkin.playCamera.addObjToUI) funkin.playCamera.addObjToUI(strumlines.opponentBg);
		else strumlines.opponentBg.setScrollFactor(0);
		...
		if (funkin.playCamera && funkin.playCamera.addObjToUI) funkin.playCamera.addObjToUI(strumlines.playerBg);
		else strumlines.playerBg.setScrollFactor(0);
	...
	static createArrow(scene, strumlines, x, y, assetKey, scale, alpha, dir, skinData, isPlayer) {
		...
		if (funkin.playCamera && funkin.playCamera.addObjToUI) funkin.playCamera.addObjToUI(arrow);
		else arrow.setScrollFactor(0);
		...
		arrow.baseScale = scale; // Guarda la escala original del JSON
	...
	static spawnNotesArray(scene, notesManager, chartNotesArray, assetKey, skinData, fallbackFrames) {
		...
		chartNotesArray.forEach((noteData) => {
			...
			const isPlayer = funkin.NoteDirection.isPlayerNote(p);
			const lane = funkin.NoteDirection.getBaseLane(dir);
			const dirName = funkin.NoteDirection.getDirectionName(lane);
			...
			if (funkin.playCamera && funkin.playCamera.addObjToUI) funkin.playCamera.addObjToUI(note);
			else note.setScrollFactor(0);
			...
			note.baseScale = scale; // Guarda la escala original del JSON
			note.hasMissed = false;
			note.wasHit = false;
			note.active = true; 

			notesManager.notes.push(note);
			if (funkin.playNotes) funkin.playNotes.emit("spawn", { note: note });
		});

		notesManager.notes.sort((a, b) => a.noteTime - b.noteTime);
		if (funkin.playNotes) funkin.playNotes.emit("allLoaded", { total: notesManager.notes.length });
	}
	...
	static checkSparrowXML(scene, assetKey) {
		if (funkin.animation && funkin.animation.sparrow) {
			const xmlText = scene.cache.text.get(`${assetKey}_rawXML`);
			if (xmlText) {
				funkin.animation.sparrow.fixPhaserSparrow(scene, assetKey, xmlText);
...
if (typeof window !== 'undefined') funkin.ArrowsSpawner = ArrowsSpawner;