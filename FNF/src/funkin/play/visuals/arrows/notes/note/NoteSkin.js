/**
 * Gestor visual de texturas, shaders y dependencias visuales de notas.
 */
class NoteSkin {
	constructor(manager) {
	...
	injectNote(noteData) {
		...
		if (funkin.ArrowsSpawner) {
			const fallbackFrames = funkin.ArrowsSpawner.createNoteFallbackFrames(
				this.scene,
				assetKey,
				skinData.animations
			);
			...
			funkin.ArrowsSpawner.spawnNotesArray(
				this.scene,
				this.manager,
				[noteData],
				assetKey,
	...
	reloadSkin() {
		...
		let fallbackFrames = {};
		if (funkin.ArrowsSpawner) {
			funkin.ArrowsSpawner.checkSparrowXML(this.scene, assetKey);
			fallbackFrames = funkin.ArrowsSpawner.createNoteFallbackFrames(
				this.scene,
				assetKey,
				skinData.animations
			);
...
if (typeof window !== "undefined") funkin.NoteSkin = NoteSkin;