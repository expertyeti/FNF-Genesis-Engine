/**
 * Parsea el charter, asigna texturas y animaciones para las colas de notas.
 */
class SustainSkin {
	constructor(manager) {
	...
	initSustains() {
		...
		if (funkin.animation && funkin.animation.sparrow) {
			...
			if (xmlText) {
				funkin.animation.sparrow.fixPhaserSparrow(this.scene, assetKey, xmlText);
	...
	spawnSustains(chartNotesArray, assetKey, skinData) {
		...
		chartNotesArray.forEach((noteData) => {
			...
			for (let i = 0; i < numPieces; i++) {
				...
				if (funkin.playCamera && funkin.playCamera.addObjToUI) funkin.playCamera.addObjToUI(piece);
			...
			if (funkin.playCamera && funkin.playCamera.addObjToUI) funkin.playCamera.addObjToUI(end);
	...
	reloadSkin() {
		...
		this.manager.sustains.forEach((sustain) => {
			const dirName = funkin.NoteDirection.getDirectionName(sustain.lane);
			const frameData = cachedFrames[dirName] ||
				cachedFrames["left"] ||
				cachedFrames["center"] || { body: "", end: "" };
...
if (typeof window !== "undefined") funkin.SustainSkin = SustainSkin;