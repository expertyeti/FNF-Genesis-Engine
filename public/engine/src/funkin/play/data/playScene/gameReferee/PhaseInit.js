/**
 * @file PhaseInit.js
 * Fase inicial encargada de inyectar dependencias, descargar recursos dinámicos
 * y construir visual y lógicamente el entorno de juego.
 */
...
class PhaseInit {
  ...
  async enter() {
    ...
    if (
      !funkin.play.chart ||
      typeof funkin.play.chart.loadChart !== "function"
    ) {
    ...
    if (!chartLoaded) {
      console.error(
        "[Genesis Engine] Error al cargar el chart. Regresando a la escena anterior.",
      );
      const sourceScene =
        scene.playData && scene.playData.sourceScene
          ? scene.playData.sourceScene
          : "MainMenuScene";
    ...
    if (
      funkin.play.characterLoader &&
      typeof funkin.play.characterLoader.loadCharacters === "function"
    ) {
    ...
    const notesAPI =
      funkin.play.visuals && funkin.play.visuals.arrows
        ? funkin.play.visuals.arrows
        : {};
    if (notesAPI.notes && notesAPI.notes.NoteDirectionManager)
      scene.noteDirectionManager = new notesAPI.notes.NoteDirectionManager(
        keyCount,
      );

    if (
      !funkin.play.uiSkins ||
      typeof funkin.play.uiSkins.loadSkinData !== "function"
    ) {
    ...
    if (
      window.funkin.playPreload &&
      typeof window.funkin.playPreload.preloadAudio === "function"
    ) {
      await window.funkin.playPreload.preloadAudio(
        scene,
        scene.playData.actuallyPlaying,
      );
    }
    ...
    if (funkin.play.visuals.stage && funkin.play.visuals.stage.PlayListSprites)
      funkin.play.playListSprites =
        new funkin.play.visuals.stage.PlayListSprites(scene);
    if (
      funkin.play.visuals.characters &&
      funkin.play.visuals.characters.AnimateCharacters
    )
      scene.animateCharacters =
        new funkin.play.visuals.characters.AnimateCharacters(scene);
    ...
    if (stage.CreateAtlasSprite)
      stage.CreateAtlasSprite.execute(scene, stageName);

    const chars = funkin.play.visuals.characters || {};
    if (chars.SparrowCharacters)
      await chars.SparrowCharacters.execute(scene, stageName);
    if (chars.AtlasCharacters)
      await chars.AtlasCharacters.execute(scene, stageName);

    if (
      notesAPI.ArrowsSpawner &&
      typeof notesAPI.ArrowsSpawner.clearExisting === "function"
    )
    ...
    if (
      notesAPI.notes &&
      notesAPI.notes.SustainNotesManager &&
      scene.strumlines
    )
      scene.sustainNotesManager = new notesAPI.notes.SustainNotesManager(
        scene,
        scene.strumlines,
      );
    if (notesAPI.notes && notesAPI.notes.NotesManager && scene.strumlines)
      scene.notesManager = new notesAPI.notes.NotesManager(
        scene,
        scene.strumlines,
      );
    ...
    if (notesAPI.ArrowsSpawner) {
      if (scene.strumlines)
        notesAPI.ArrowsSpawner.spawnStrumlines(scene, scene.strumlines);
      if (scene.notesManager)
        notesAPI.ArrowsSpawner.spawnChartNotes(scene, scene.notesManager);
    }
    ...
    if (ui.JudgmentPopUpManager)
      scene.judgmentPopUpManager = new ui.JudgmentPopUpManager(scene);
    if (ui.ComboPopUpManager)
      scene.comboPopUpManager = new ui.ComboPopUpManager(scene);
    if (notesAPI.notes && notesAPI.notes.NoteSplashesManager)
      scene.noteSplashesManager = new notesAPI.notes.NoteSplashesManager(scene);
    if (notesAPI.notes && notesAPI.notes.HoldCoversManager)
      scene.holdCoversManager = new notesAPI.notes.HoldCoversManager(scene);
    if (funkin.play.input && funkin.play.input.PlayInput)
      scene.inputHandler = new funkin.play.input.PlayInput(scene);
    ...
    funkin.play.health = {
      ...
      update: function (time, delta) {
        ...
        this.healthLerp = Phaser.Math.Linear(
          this.healthLerp,
          this.health,
          lerpFactor,
        );
    ...
    // ==========================================
    // REPRODUCCIÓN DEL SONIDO DE MISS Y CASTIGO
    // ==========================================
    scene.events.on("noteMiss", (data) => {
      if (!window.autoplay) {
        ...
        if (scene.cache.audio.exists(missSound)) {
          scene.sound.play(missSound, {
            volume: Phaser.Math.FloatBetween(0.7, 0.9),
          });
        }

        // Mutea las vocales del lado que falló
        if (scene.songPlaylist && scene.songPlaylist.vocalManager) {
          if (
            typeof scene.songPlaylist.vocalManager.muteVocals === "function"
          ) {
            scene.songPlaylist.vocalManager.muteVocals(data.isPlayer);
          }
        }
      }
    });
    ...
    const bpm =
      funkin.play.chart && typeof funkin.play.chart.get === "function"
        ? funkin.play.chart.get("metadata.audio.bpm") || 120
        : 120;

    if (window.funkin.conductor) {
      if (
        window.funkin.conductor.bpm &&
        typeof window.funkin.conductor.bpm.set === "function"
      )
        window.funkin.conductor.bpm.set(bpm);
      else window.funkin.conductor.bpm = bpm;
