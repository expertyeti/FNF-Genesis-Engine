/**
 * @file src/funkin/play/visuals/arrows/spawner/ArrowsSpawner.js
 * Centraliza toda la lógica de instanciación y creación de flechas y strumlines.
 */
...
const spawner = window.funkin.play.visuals.arrows.spawner;

// ============================================================================
// UTILIDADES Y CÁLCULOS
// ============================================================================

spawner.calculateOptimalPoolSize = function(chartData, screenHeight, scrollSpeed, scrollConstant = 0.45, safetyMargin = 1.15) {
...
spawner.checkSparrowXML = function(scene, assetKey) {
...
spawner.clearExisting = function(scene) {
  if (scene.strumlines) {
...
spawner.createNoteFallbackFrames = function(scene, assetKey, prefixes) {
...
spawner.createStrumAnimations = function(scene, assetKey, animsData, directions) {
  const frameNames = scene.textures.get(assetKey).getFrameNames();
...
spawner.createArrow = function(scene, strumlines, x, y, assetKey, scale, alpha, dir, skinData, isPlayer) {
...
spawner.spawnNotesArray = function(scene, notesManager, chartNotesArray, assetKey, skinData, fallbackFrames) {
...
spawner.spawnChartNotes = function(scene, notesManager) {
  ...
  const optimalPoolSize = spawner.calculateOptimalPoolSize(chartData, screenHeight, scrollSpeed);

  console.log(
    `[ArrowsSpawner] Pool Size Calculado:`,
    `Max Simultaneous: ${Math.ceil(optimalPoolSize / 1.15)}, Total con Margen: ${optimalPoolSize}, Total Notas: ${chartData.length}`
  );
  ...
  const fallbackFrames = spawner.createNoteFallbackFrames(scene, assetKey, skinData.animations);
  
  spawner.spawnNotesArray(scene, notesManager, chartData, assetKey, skinData, fallbackFrames);
};

spawner.spawnStrumlines = function(scene, strumlines) {
  ...
  spawner.checkSparrowXML(scene, assetKey);
  spawner.createStrumAnimations(scene, assetKey, skinData.animations, strumlines.directions);
  ...
  for (let i = 0; i < strumlines.keyCount; i++) {
    ...
    const oppArrow = spawner.createArrow(scene, strumlines, 0, 0, assetKey, scale, alpha, dir, skinData, false);
    strumlines.opponentStrums.push(oppArrow);

    const playerArrow = spawner.createArrow(scene, strumlines, 0, 0, assetKey, scale, alpha, dir, skinData, true);
