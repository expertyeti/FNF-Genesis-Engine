class ComboPopUpManager {
        ...
        if (funkin.playNotes) {
            funkin.playNotes.event('noteHit', (hitData) => {
                ...
                if ((isMyNote || is2P) && hitData.pressed && hitData.judgment !== 'miss') {
                    ...
                    if (is2P) {
                        currentCombo = hitData.isPlayer ? 
                            (funkin.playerStaticsInSong ? funkin.playerStaticsInSong.combo : 0) :
                            (funkin.player2StaticsInSong ? funkin.player2StaticsInSong.combo : 0);
                    } else {
                        currentCombo = funkin.playerStaticsInSong ? funkin.playerStaticsInSong.combo : 0;
                    }

                    if (currentCombo > 0) {
                        this.spawnCombo(currentCombo, hitData.isPlayer, is2P);
                    }
                }
            });
        }
    ...
    spawnCombo(comboNum, isPlayerSide, is2P) {
...
funkin.play.visuals.ui.ComboPopUpManager = ComboPopUpManager;