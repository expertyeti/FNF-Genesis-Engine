/**
 * @file MenuOptionSprite.js
 * Sprite personalizado para las opciones del menú principal.
 */
class MenuOptionSprite extends Phaser.GameObjects.Sprite {
  /**
   * @param {Phaser.Scene} scene - Escena actual.
   * @param {Object} data - Datos de configuración del sprite.
   */
      ...
      if (animData.indices && animData.indices.length > 0) {
        frames = animData.indices
          .map((i) => {
            ...
            return allFrames.find((f) =>
              f.startsWith(`${animData.name}${padded}`),
            );
          })
          .filter(Boolean);
      } else {
...
funkin.ui.mainMenu.MenuOptionSprite = MenuOptionSprite;
